...
public class Exercises {
	 ...
	 *
	 * "KITCHEN4001" -> 0.20 "GARAGE1070" -> 0.15 "" -> 0.10 "KITCHEN6073" -> 0.40
	 * "BEDROOM3434" -> 0.60 "BATH0073" -> 0.15
	 *
	 * The item number should be case insensitive so "kitchen4001", "Kitchen4001",
	 * and "KITCHEN4001" should all retuLIVINGROOMrn 0.20.
	 *
	...
	public Double isItOnSale(String itemNumber) {
		...
		onSale.put("KITCHEN4001", .20);
		onSale.put("GARAGE1070", .15);
		onSale.put(" ", .10);
		onSale.put("KITCHEN6073", .40);
		onSale.put("BEDROOM3434", .60);
		onSale.put("BATH0073", .20);

		String key = itemNumber.toUpperCase();

		if (onSale.containsKey(key)) {
			return onSale.get(key);
		} else {
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		Map<Integer, Integer> littleCousin = new HashMap<>();

		for (int number : ints) { // iterating through

			if (!littleCousin.containsKey(number)) { // review again. Need to understand concept.
				littleCousin.put(number, 1);
			}

			else {
				littleCousin.put(number, littleCousin.get(number) + 1);
			}
		}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		Map<String, Boolean> trueOnly = new HashMap<>();

		for (String word : words) {

			if (!trueOnly.containsKey(word)) {
				trueOnly.put(word, false);
			}

			else {
				trueOnly.put(word, true);
			}
		}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		Map<String, Integer> turnedAround = new HashMap<String, Integer>();
	...
	public List<String> distinctValues(List<String> stringList) {
		...
		for (String currentString : stringList) {
			distinctValues.add(currentString); 
		}

		List<String> distinctStringList = new ArrayList<String>();
		for (String currentString : distinctValues) {
			distinctStringList.add(currentString);
		}

		return distinctStringList;
		
		//too tired to think. Not certain how I even got this far. Luck for sure.
	}
}
